---
name: electron
description: Automate Electron desktop apps (VS Code, Slack, Discord, Figma, Notion, Spotify, etc.) using agent-browser via Chrome DevTools Protocol. Use when the user needs to interact with an Electron app, automate an Electron desktop app, connect to a running Electron app, or test an Electron application. Triggers include "automate Slack app", "control VS Code", "interact with Discord app", "test this Electron app", "connect to desktop app", or any task requiring automation of an Electron application.
allowed-tools: Bash(agent-browser:*), Bash(npx agent-browser:*)
---

# Electron App Automation

Automate Electron desktop apps using agent-browser. Electron apps are built on Chromium and can expose a Chrome DevTools Protocol (CDP) port when launched with remote debugging enabled, which lets agent-browser use the same snapshot-interact workflow used for web pages.
...
Electron supports the `--remote-debugging-port` flag because it inherits Chromium's debugging infrastructure.
...
Use an explicit port for Electron apps. `--auto-connect` is primarily documented for auto-discovering Chrome, so it should be treated as a fallback rather than the default Electron workflow.
...
- If the snapshot is noisy, try `agent-browser snapshot -i -c` to keep interactive elements while removing empty structural nodes
...
Most Electron apps can be automated this way if you can relaunch them with `--remote-debugging-port` and their UI is exposed through a CDP-visible target.
